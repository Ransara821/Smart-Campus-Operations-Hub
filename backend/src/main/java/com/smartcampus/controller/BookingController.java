...
public class BookingController {
    ...
    public ResponseEntity<List<Booking>> getAllBookings(@AuthenticationPrincipal OAuth2User principal) {
        ...
        for (Booking b : bookings) {
            String qrStatus = (b.getQrValidationData() != null && !b.getQrValidationData().isEmpty())
                    ? "✓ (" + b.getQrValidationData() + ")"
                    : "✗ (null)";
    ...
    public ResponseEntity<?> getMyBookings(@AuthenticationPrincipal OAuth2User principal) {
        ...
        for (Booking b : bookings) {
            System.out.println("  - Booking " + b.getId() + ": status=" + b.getStatus() + ", qrData="
                    + (b.getQrValidationData() != null ? "✓" : "✗"));
        }
        return ResponseEntity.ok(bookings);
    }
    ...
    @GetMapping("/{id}/qr")
    public ResponseEntity<?> getBookingQrDetails(@PathVariable String id,
            @AuthenticationPrincipal OAuth2User principal) {
    ...
    public ResponseEntity<?> verifyQrCode(
        ...
        System.out.println(
                "🔍 QR Verification Request - qrData: " + qrData + ", userId: " + userId + ", studentId: " + studentId);
        ...
        // Check if booking time window is valid (allow check-in 15 mins before start
        // time)
        ...
        if (userId != null && !userId.isEmpty()) {
            ...
            System.out.println("🔍 Check-in request - BookingId: " + booking.getId() + ", Student ID: '" + studentId
                    + "', UserName: '" + normalizedUserName + "'");

            // Check for duplicate userId (Student ID or Email) - PRIMARY CHECK
            Optional<Attendance> existingByUserId = attendanceRepository.findByBookingIdAndUserId(booking.getId(),
                    userId);
            ...
            if (existingByUserId.isPresent()) {
                ...
                System.out.println("   Existing record: Name='" + existing.getUserName() + "', CheckedInAt="
                        + existing.getCheckedInAt());
                System.out.println("   Attempted Name: '" + normalizedUserName + "'");

                // Create clear message based on whether studentId was used
                String duplicateMessage;
                if (existing.getStudentId() != null && !existing.getStudentId().isEmpty()) {
                    duplicateMessage = "This Student ID (" + existing.getStudentId()
                            + ") has already been used to check in";
            ...
            // Check by name (case-insensitive) - SECONDARY CHECK (for users who may not
            // have userId)
            Optional<Attendance> existingByName = normalizedUserName.isEmpty() ? Optional.empty()
            ...
            if (existingByName.isPresent()) {
                ...
                System.out.println("   Existing record: UserId='" + existing.getUserId() + "', CheckedInAt="
                        + existing.getCheckedInAt());
            ...
            System.out.println("   Progress: " + totalAttendees + "/"
            ...
            response.put("expectedAttendees",
                    booking.getExpectedAttendees() != null ? booking.getExpectedAttendees() : 0);
    ...
    public ResponseEntity<?> getBookingAttendance(@PathVariable String id) {
        ...
        System.out.println("   Attendance: " + totalAttendees + "/"
                + (booking.getExpectedAttendees() != null ? booking.getExpectedAttendees() : 0));
